...
$(document).ready(() => {
    ...
    function onClassName({ path, field, value }) {
        ...
        if (field !== undefined) {
            ...
            if (
                compareJson &&
                isValidJson(jsonEditorLeft.getText()) &&
                isValidJson(jsonEditorRight.getText())
            ) {
...
$(window).on("load", async () => {
    await alertIntroMsg(Tool.INTRO_WARN_MSG, true, Tool.INTRO_SHOW_TIME);
});
...
async function getDiffs() {
    if (
        ...
        if (aLines.length !== bLines.length) {
            alertWebMsg(
                "Not mached!! Two JSON's have different lengths!!",
                false,
                false
            );
            ...
            alertWebMsg(
                "Not matched!! Two JSON's have the same length, but there are different!!",
                false,
                false
            );
        }
...
async function highlightDiffsInCodeMode() {
    ...
    if (compareClicked) {
        ...
        if (diffs) {
            ...
            while (!index.done) {
                if (
                    !diffs.common.has(index) &&
                    index.value + 1 - startLine_1 >= 0
                ) {
                    $(`#${idLeft} .ace_line_group`)
                        .eq(index.value + 1 - startLine_1)
                        .addClass("different_element");
                }
            ...
            while (!index.done) {
                if (
                    !diffs.common.has(index) &&
                    index.value + 1 - startLine_2 >= 0
                ) {
                    $(`#${idRight} .ace_line_group`)
                        .eq(index.value + 1 - startLine_2)
                        .addClass("different_element");
                }
                index = rightIndexes.next();
            }
